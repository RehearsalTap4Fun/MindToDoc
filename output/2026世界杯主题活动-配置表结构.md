...
- **用途**：各切片类型 FSM 流程参数与操作引擎默认值  
- **关联**：按 SliceType 读取；与开发对接说明 §4.1 同源  
- **填表**：关卡

| 字段 | 类型 | 读取端 | 说明 | proto/示例 |
|------|------|--------|------|------------|
| ID | int | c | 切片流程ID | — |
| SliceType | string | c | 切片类型（OpenPlay/FreeKick/Penalty/Corner/ThrowIn/Goalkeeper） | — |
| DefaultAimMode | string | c | 进入切片时强制重置的默认操作模式（Draw/Drag/留空=沿用上一切片）；守门切片走独立 Performer，本字段无效 | 任意球=Draw、点球=Drag、其他留空 |
| HoldStartRadius | float | c | 起手区半径（按开发约定单位）；玩家在以足球为中心、半径内任意点按下视为起手 | — |
| AimYawRange | float | c | 扇形张角（度）；划线落点 / 弹弓出脚反向延长线方向均受此约束 | — |
| CamYawRange | float | c | Ready 期相机粗瞄环视范围（度）；点球几乎为 0、运动战放开 | — |
| CamYawDefault | float | c | Ready 期相机默认朝向（相对球→推荐目标的偏角，度） | — |
| PowerRangeFactor | float | c | 满力射程 = 推荐目标距离 × 此值（默认 1.2，可逐切片覆盖） | — |
| WaitInputTimeMs | int | c | 等待限时(0=无限)；非守门切片为 0，守门切片填反应时长 | — |
| SaveAngleThresholdDeg | float | c | 守门切片专属：玩家划线方向与来球方向夹角 ≤ 此值 = 扑救成功 | — |
| MaxSaveDistance | float | c | 守门切片专属：扑救距离最大值（划线力度满档对应的扑救范围） | — |
