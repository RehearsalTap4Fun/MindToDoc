...
- **参考游戏对标**：足球生涯养成类（签约、试训、赛季目标）\+ 切片式休闲操作（单次进攻/守门手动操作）。竞猜赔率参考内部"X1 竞猜系统"（资料缺失，见待确认）。
...
| players\_init | json | 双方球员初始位置/朝向列表 | 公共摆位 |
| camera | json | 默认视角 | instance 可 override |
| target\_point | vec3 | 默认目标点/落点 | 守门可空 |
| operable\_angle | float | 默认可操作夹角 | 非守门 |
...
| 双方球队名、角色模型、星级、排名 | 展示对阵信息；排名和星级只展示，不由前端计算。 |
...
| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| ai\_profile\_id | int | AI档位ID | ≥1 |
| difficulty | enum | 难度等级 | easy/normal/hard… TODO |
| param\_overrides | json | 对切片权重参数的覆盖（门将权重/跑位模板等） | TODO |
...
- **正赛 / 单淘汰阶段（7.13–7.18，固定 6 个比赛日，单败淘汰）**：
    ...
    - **每个比赛日固定三段流程**：**① 备战** → **② 当轮下注** → **③ 比赛结算**。
        - **备战**：当日比赛开始前的准备窗口，玩家可调整携带道具/查看对手与赔率（具体可操作项 TODO）。
        - **当轮下注**：开放本轮竞猜，按轮次显示文案「64 强下注 / 32 强下注 / 16 强下注 / 8 强下注 / 4 强下注 / 决赛下注」；规则见 \[3.7\]。
...
- **下注窗口**：从**明确对手后**开放，到**该场比赛结算前**关闭，期间均可取消或修改下注。
- **下注内容**：仅猜胜负，即**选择晋级方**；可押注**自己参赛的队伍**（参赛后组队不可变更）。
- **结算派彩**：比赛结算后，押中晋级方者按**下注量 × 赔率**获得竞猜币；押错损失投注。
- **赔率**：采用其他项目组现成赔率计算方式（**TODO**，接入时对接）。
- **竞猜币来源**：每日发放免费竞猜币；礼包（\[3.11\]）也投放竞猜币；竞猜失败会回收80%下注金额。
- **消耗去向**：竞猜币用于下注；亦可在 \[3.10 兑换商店\]（即"竞猜币兑换商店"，不单列）兑换局外道具。
- **排名数据**：本系统统计每位玩家的**命中次数**与**总收益**，排名榜在 \[3.8\] 统一处理。
- **支持模块**：提供当前可下注场次列表、已下注未结算列表、历史下注列表
...
| 场次卡：Quarterfinals、in progress 倒计时 | 展示当前可竞猜阶段和截止倒计时。 |
| 双方信息、赔率 1.33 / 1.45、排名 #1/#2、投注人数 | 赔率、投注人数、队伍信息由服务端下发。 |
| Betting 按钮 / Change 按钮 | 未下注时显示下注；已下注时显示修改。截止后置灰。 |
| 下注弹窗 Betting options | 显示双方、当前选择、赔率、投注人数。 |
| 投注额按钮：100、200、500、1000、5000 | 快捷投注档位；不能超过竞猜币余额。 |
| Corresponding rewards \+133 | 预计返还/收益预览；按服务端赔率计算。 |
| 竞猜结果页：Top 16 betting settlement、\+10,000、Results | 结算后展示收益、命中状态。 |
| 结果卡：VICTORY、播放按钮、\+13,330、View Video / Continue | 命中展示收益；播放按钮看回放；继续关闭结算。 |
| 底部导航：shop、Historical Matches、My guess、Profit ranking | 映射为商店、历史下注/历史比赛、我的竞猜、收益榜。 |
...
| odds\_a / odds\_b | float | 双方赔率 | TODO（接现成算法） |
| open\_time / close\_time | datetime | 开放/截止（截止=结算前） | TODO |
...
| stake | int | 投注竞猜币 | ≥1 |
| payout | int | 派彩（命中=stake×odds，否则0） | ≥0 |
| hit | bool | 是否命中 | - |
...
- [ ] **3.2 切片操作核心**： 划线/弹弓·守门强制划线·首个非守门切片锁定整关/6类切片/3道具/回溯重置·三层配置(type\_def/preset/instance)\+objectives/modifiers正交·叠加优先级固定·**震动模块(关键结果+核心操作·分级强度×图案·全局映射表haptic\_config·总开关默认开·平台降级不阻塞)**
...
- [ ] **3.6 淘汰赛**： 中后期开放·四阶段赛程(组队7.10–7.11/海选7.12/正赛单淘汰6轮7.13–7.18/展示期7.19)·海选均分G组取前K=64强(不开竞猜·仅见己组)·正赛每日一轮规模减半64→32→16→8→4→决赛·每日三段(备战→X强下注→比赛统一结算)·演算闭环(切片→比分/平局破法/轮空/名次)·回放仅展示·不参与排名
- [ ] **3.7 竞猜**： 淘汰赛非轮空场·选晋级方·可押己队·赔率TODO·产出命中/收益
