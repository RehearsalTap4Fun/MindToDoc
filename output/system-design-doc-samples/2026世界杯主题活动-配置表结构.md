...
> **列级 SSOT**：以 `output/test-config/ActivitySoccer_preview.xlsx` 页签表头为准（当前 **26** 个策划页签）；本文由 `generate_config_tables_doc.py` 自动生成字段表。
> **生成时间**：脚本随测试配置同步更新。
...
| 文件 | 说明 | 策划维护 |
|------|------|----------|
| ActivitySoccer＿preview.xlsx | 活动玩法主配置（26 个策划页签） | 数值 / 关卡 / 活动 |
...
| ID | int | cs | 配置ID | — |
| Type | string | cs | 类型 | — |
| ContentID | string | cs | 关联ActiveRankSection | — |
