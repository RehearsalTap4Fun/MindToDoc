...
SHEET_META: dict[str, dict[str, str]] = {
    "ActvSoccerCharacterCfg": {"用途": "4 角色外观、球衣与号码", "关联": "→ 创角流程", "填表": "数值+美术"},
    "ActvSoccerNationalityCfg": {"用途": "国籍与首签候选合同池", "关联": "ContractPool→ContractCfg；NameLcKey→Language", "填表": "数值"},
    "ActvSoccerGuideCfg": {"用途": "试训、签约后强引导与第一关新手引导步骤", "关联": "DialogueLcKey→Language；NextID串联步骤", "填表": "关卡/体验"},
    "ActvSoccerSlicePresetCfg": {"用途": "切片摆位预设（L2 主资产）", "关联": "SliceType；被 SliceInstanceCfg.PresetID 引用", "填表": "关卡"},
    "ActvSoccerSliceInstanceCfg": {"用途": "切片实例装配（L3;含胜利目标/AI/机制/夹角/视角）", "关联": "←Preset；→LevelCfg.SliceList", "填表": "关卡"},
    ...
    "ActvSoccerLevelCfg": {"用途": "关卡:切片序列、胜平阈值、门票、对手;FortID 供UI跨轮连续编号", "关联": "SliceList→Instance；AiProfileID→Profile；SeasonID→SeasonCfg", "填表": "关卡+数值"},
    ...
    "ActvSoccerFameGrowthLevelCfg": {"用途": "知名度等级、许可、主角评分、金币加成", "关联": "PlayerRating→淘汰赛主角评分；档位可拆细；ContractStarLicReward→ContractStarLicCfg", "填表": "数值"},
    "ActvSoccerLifeGrowthLevelCfg": {"用途": "生活等级、门票 buff、许可、图标名称", "关联": "TicketCap/Recover 与附录常量叠加；不参与主角评分", "填表": "数值+美术"},
    "ActvSoccerTeamCfg": {"用途": "球队展示（队名/队服/队标/国旗）", "关联": "←ContractCfg.TeamID；←LevelCfg.OpponentTeamID", "填表": "数值+美术"},
    "ActvSoccerContractStarLicCfg": {"用途": "联赛换约星级权重", "关联": "←Growth 两线 ContractStarLicReward", "填表": "数值"},
    "ActvSoccerContractCfg": {"用途": "合同待遇、赛季目标、发放场景、球衣/目标描述展示", "关联": "TeamID→Team；首签←Nationality；换约←StarLic", "填表": "数值+美术"},
    "ActvSoccerSeasonCfg": {"用途": "联赛轮次、NextSeason 链", "关联": "ID=LevelCfg.SeasonID", "填表": "数值"},
    ...
    "ActvSoccerMatchRewardCfg": {"用途": "淘汰赛晋级/出局奖励与邮件", "关联": "Turn=KnockoutPhaseCfg.ID；MailID→全局邮件", "填表": "数值+活动"},
    "ActvSoccerBetMultiplierCfg": {"用途": "胜率×战力比→左右赔率二维对照表(=程序ChampionOddsCfg)", "关联": "同步匹配后按 (WinRate, PowerRate) 查行→OddsLeft/OddsRight 万分值；配合附录 bet_* 常量", "填表": "数值"},
    "ActvSoccerBetStakeTierCfg": {"用途": "投注档位(免费+5档)", "关联": "下注弹窗选项；免费档HitPayout=5", "填表": "数值"},
    "ActvSoccerAchieveCfg": {"用途": "活动成就(类型/计数/品质/图标/标题/描述)", "关联": "Type/Count 定义解锁条件；Title/Des 直接展示", "填表": "数值+美术"},
    "ActvSoccerRankCfg": {"用途": "活动排行榜页签配置(图标/背景/关联段位表)", "关联": "ContentID→通用 ActivityRank/ActvRankSectionCfg", "填表": "活动+美术"},
    "ActvSoccerCharacterStateCfg": {"用途": "主角/球员表现状态→动画 key 映射(FSM)", "关联": "客户端 FSM 状态机与美术动作;不进策划数值填表", "填表": "客户端+美术"},
...
SECTION_GROUPS: list[tuple[str, list[str]]] = [
    ("创角与引导", ["ActvSoccerCharacterCfg", "ActvSoccerNationalityCfg", "ActvSoccerGuideCfg"]),
    (
        "切片与关卡",
        [
    ...
    ("成就与排行", ["ActvSoccerAchieveCfg", "ActvSoccerRankCfg"]),
...
def build_doc(summary: dict, sheet_columns: dict[str, list[dict]], sheets: list[str]) -> str:
    ...
    lines: list[str] = [
        ...
        table_row("客户端表现映射(FSM 状态→动画)", "ActvSoccerCharacterStateCfg", "客户端+美术维护;虽在活动 xlsx 中,读取端仅 c,不由数值策划直接编辑数值"),
    ...
    lines += [
        ...
        "    GD[GuideCfg] -.引导.- SI[SliceInstanceCfg]\n",
        "  end\n",
        "  subgraph 切片关卡\n",
        ...
        "  end\n",
        "  subgraph 淘汰竞猜\n",
        "    KO[KnockoutCfg] --> KP[KnockoutPhaseCfg]\n",
        "    KP --> MR[MatchRewardCfg]\n",
        "    KP -->|BetOpen| BM[BetMultiplierCfg]\n",
        "    BM --> BT[BetStakeTierCfg]\n",
        "  end\n",
        "  subgraph 通用配套\n",
        "    BP[BattlePassNew]\n",
        "    GF[GiftCfg]\n",
        ...
        table_row("引导→切片", "GuideCfg.NextID 串联步骤;GestureDesc/DialogueLcKey 驱动指引"),
        table_row("关卡组装", "LevelCfg.SliceList array → SliceInstanceCfg；AI 字段已并入实例表"),
        table_row("联赛轮次", "SeasonCfg.ID = LevelCfg.SeasonID；NextSeason 链"),
        ...
        table_row("淘汰赛→竞猜", "KnockoutPhaseCfg.BetOpen 排期 → 服务端 bet_match；演算胜率×战力比 → BetMultiplierCfg + bet_＊ 常量"),
    ...
    lines += [
        ...
        "- 通用 BP / 商店 / 礼包各档数值\n",
        "- 切片实例 AimYawRange/CamYawRange 曲线、AI 难度档\n",
        "- 附录常量合并进 `dataconfig/ConstConfig.xlsx`（含 `ActvSoccer_bet_*`）\n",
        "- BetMultiplierCfg(=ChampionOddsCfg) 二维赔率网格、BetStakeTier 6 档投注\n\n",
        "---\n\n",
        "## 测试配置与正式表\n\n",
