...
> **列级 SSOT**：以 `output/test-config/ActivitySoccer.xlsx` 页签表头为准（当前 **26** 个策划页签，含程序侧新增接球决策配置需求）；本文由 `generate_config_tables_doc.py` 自动生成字段表。
> **生成时间**：脚本随测试配置同步更新。
...
| `ActvSoccerBetMultiplierCfg` | 胜率→奖励倍率对照表(=程序ChampionOddsCfg) | 同步匹配后查最近WinRatePctA；配合附录bet_*常量 | 数值 |
...
| PlayerStyleCfgID | int | cs | 球员风格→ActvSoccerPlayerStyleCfg；0=按角色兜底 | — |
...
| Group | int | cs | 所属联赛轮次(=SeasonCfg.ID) | — |
...
| 字段 | 类型 | 读取端 | 说明 | proto/示例 |
|------|------|--------|------|------------|
| ID | int | cs | currency_id | — |
| CurrencyKey | string | cs | 货币键 | — |
| Usage | string | cs | 用途 | — |
...
| Group | int | cs | 联赛系列(相同Group条数=总轮次) | — |
...
|------|------|--------|------|------------|
| ID | int | cs | 编号 | — |
| WinRatePctA | int | cs | A方胜率(%) | — |
| MultA | float | cs | A方奖励倍率 | — |
| WinRatePctB | int | cs | B方胜率(%) | — |
| MultB | float | cs | B方奖励倍率 | — |
| ReturnRate | float | cs | 参考返还率(策划校验) | — |
