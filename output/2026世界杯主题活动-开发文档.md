...
| pos | vec3 | 初始站位 | 球场坐标 |
| facing | float | 初始朝向（°，水平面 yaw；+Z 为 0°） | 摆位配置必填 |
...
- **演算模型**（海选与单淘汰共用，公式先拍、后期数值策划定，完整口径见数据表 `match_simulation_rule`）： 
    - 单主角评分 = `fame_growth_level[当前知名度等级].player_rating`（仅知名度表；生活等级不投放评分）。
...
| 项 | 说明 |
|---|------|
| single\_rating = fame\_growth\_level[fame\_lv].player\_rating | 单主角评分，仅知名度线；`fame_lv` 档位可拆细 |
...
| 结果卡：VICTORY、播放按钮、+13,330、Continue | 命中展示收益；继续关闭。 |
...
> 特效指游戏内由粒子 / 着色器 / 光效实现的即时反馈，区别于 5.7 动效与反馈 的 UI / 镜头 / Lottie 表现层。移动端竖屏需控制同屏粒子数与 overdraw，不得遮挡操作区、轨迹层与结算按钮。

#### 局内 · 球与射门

| VFX ID | 触发时机 | 表现要点 | 优先级 | 关联音效 |
|--------|----------|----------|--------|----------|
| VFX_AIM_LINE | 玩家进入可操作态后辅助线持续显示，划线/弹弓操作过程中实时更新 | 轨迹流光 + 落点呼吸光圈 + 可操作夹角扇形；**划线模式**：随手指移动实时刷新轨迹；**弹弓模式**：随拖拽方向实时更新预瞄线；复用 3.3 辅助线资产，VFX 负责动态部分 | P0 | — |
| VFX_BALL_PULL | 弹弓模式：玩家按下开始拖拽到松手出脚 | 橡皮筋拉伸光带（随拖拽距离增长）、方向预瞄线、松手瞬间回弹消散；**仅弹弓模式触发**，划线模式不触发 | P0 | SFX_BALL_RELEASE |
| VFX_KICK_BURST | 出脚瞬间（玩家释放划线/弹弓，球离脚一帧） | 脚下草屑迸溅 + 力量冲击波；**射门（进攻/任意球/点球/角球/界外球出脚）强度高于传球**；**守门切片中对方 AI 出脚时同样触发**，但强度固定为"射门"档 | P0 | SFX_KICK |
| VFX_BALL_TRAIL | 球飞行中（从出脚到落点全程） | 速度线 / 弧光拖尾；**射门（强）**：明显光弧，随球速衰减；**传球（弱）**：淡色短尾，保留方向感；守门切片对方射门复用射门强度 | P0 | SFX_KICK |
| VFX_NET_RIPPLE | 球触网（进球判定成立、球碰到球网的帧） | 球网受冲击波动 + 落网瞬间涟漪扩散；**与 VFX_GOAL_BURST 同帧触发** | P0 | SFX_SLICE_SUCCESS |
| VFX_POST_SPARK | 球击中门框（球与门框碰撞检测命中） | 金属火花 + 轻微震屏；**击中门框为独立判定事件**（既非进球也非被扑），特效强度介于成功与失败之间；与震动事件 `hit_post`（medium+double）对应 | P1 | SFX_HIT_POST |

#### 局内 · 守门

| VFX ID | 触发时机 | 表现要点 | 优先级 | 关联音效 |
|--------|----------|----------|--------|----------|
| VFX_GK_DIVE_TRAIL | 门将判断方向并起跳扑救（玩家或 AI 做出扑救动作的帧） | 手套 / 身体残影，**左右方向必须可读**（残影朝扑救方向延伸）；扑救错误方向时同样触发，便于玩家确认操作已被识别 | P0 | SFX_GK_DIVE |
| VFX_GK_CATCH | 门将手套接触球（扑救成功判定帧） | 手套触球冲击波 + 短亮光；仅扑救成功时出现，扑救失败（球进网）不触发 | P0 | SFX_GK_SAVE_SUCCESS |
| VFX_GK_TIMER | 守门切片反应倒计时进行中（玩家处于等待扑救状态） | **UI 光效**（本条例外归入 VFX 章节统一管理）：进度条随时间缩减 + 末段（最后 30% 时长）告警闪烁；超时判定前一帧停止，进入 VFX_SLICE_FAIL | P0 | SFX_GK_TICK |

#### 局内 · 切片成败反馈

| VFX ID | 触发时机 | 表现要点 | 优先级 | 关联音效 |
|--------|----------|----------|--------|----------|
| VFX_GOAL_BURST | **进球**：切片成功且 objective 为 score，球入网判定通过 | 光爆 + 星屑 + 短促彩带，镜头微推；**助攻传球成功（pass_to objective）不触发本条**，仅触发 VFX_BALL_TRAIL 弱化版 + 队友接球动作 | P0 | SFX_SLICE_SUCCESS |
| VFX_SLICE_FAIL | 切片失败：被门将扑出（saved）/ 踢出界外（out_of_bounds）/ 守门超时（gk_timeout）/ 守门判断错误（gk_wrong_judge） | 灰暗下沉 + 烟尘散开；**失败原因必须可辨识**：被扑出时叠加 VFX_GK_CATCH，踢出界外时球越线后烟尘在界线处触发，守门失败时在 VFX_GK_TIMER 结束后触发 | P0 | SFX_SLICE_FAIL |
| VFX_REWIND | 玩家点击"倒带重试"确认回溯，场景开始复位的帧 | 时间倒流扫描线（从当前球位向切片初始位反向扫）+ 球员与足球残影回退（**须为时间倒流方向，即动作反向播放或残影从当前位反向运动到初始位**）+ 闪白复位到 players_init 状态；**所有其他正在播放的局内 VFX（VFX_BALL_TRAIL / VFX_GOAL_BURST 等）在本特效触发首帧立即中断清场，不得叠加** | P0 | SFX_ITEM_REWIND |
| VFX_WHISTLE | 哨子道具生效（玩家携带哨子进入关卡，关卡开始时自动触发） | 哨子图标高亮 + 声波扩散环 + "+1 切片"飘字光；一次性，不循环 | P1 | SFX_ITEM_WHISTLE |

#### 养成 / 商业化 / 竞猜

| VFX ID | 触发时机 | 表现要点 | 优先级 | 关联音效 |
|--------|----------|----------|--------|----------|
| VFX_REWARD_FLY | 奖励到账 / 玩家手动领取（关卡结算发放、BP 领取、排名发奖等全活动通用） | 奖励图标从来源位飞入背包资产区 + 拖尾 + 到账微光；**所有奖励到账场景统一复用此特效，来源位因场景不同（结算弹窗 / BP 轨道 / 排名榜）** | P0 | SFX_REWARD_POP |
| VFX_LEVEL_UP | 知名度 / 生活 / BP 任意等级线升级（等级数字跳变的帧） | 升级光柱 + 星屑迸发；三条等级线共用同一特效，不单独区分 | P1 | SFX_LEVEL_UP |
| VFX_CONTRACT_SEAL | 新合同发放弹窗出现（每完成一个大关卡触发） | 盖章冲击动画 + 金光从印章向外扩散 | P1 | SFX_CONTRACT_NEW |
| VFX_ACHIEVEMENT | 成就解锁（达成任意成就条件判定通过的帧） | 成就奖章飞入 + 徽光扩散 | P1 | SFX_ACHIEVEMENT_UNLOCK |
| VFX_BET_WIN | 竞猜结算后玩家本场押中晋级方（派彩入账时） | 金币雨 + jackpot 光爆；与 VFX_REWARD_FLY 同帧触发（前者表达派彩到账，后者表达视觉庆祝） | P1 | SFX_BET_WIN |
| VFX_BET_LOSE | 竞猜结算后玩家本场未押中（投注竞猜币损失时） | 筹码灰散消融，克制不夸张 | P1 | SFX_BET_LOSE |

#### 淘汰赛 / 排名 / 氛围 / 转场

| VFX ID | 触发时机 | 表现要点 | 优先级 | 关联音效 |
|--------|----------|----------|--------|----------|
| VFX_ADVANCE | 海选或单淘汰演算结果出来、本队晋级确认的帧 | 队伍卡上升光带 + 淘汰赛树对应节点点亮；晋级方向（向上/向右）需与 UI 对阵树方向一致 | P1 | SFX_MATCH_WIN |
| VFX_ELIMINATE | 演算结果出来、本队淘汰出局的帧 | 对阵树节点碎裂 / 熄灭；与 VFX_ADVANCE 互斥，同一场只触发其中一个 | P1 | SFX_MATCH_OTHER |
| VFX_CHAMPION | 展示期冠军结算 / 玩家查看自己名次包装页 | 聚光 + 礼花 + 奖杯光晕；名次越高强度越强，亚军及以下降级为 VFX_CONFETTI | P1 | SFX_MATCH_WIN |
| VFX_CAMERA_FLASH | 签约亮相动画播放期间 / 夺冠结算页展示时 | 看台随机点状闪光灯，散点分布，频率低于 3Hz（避免光敏警告） | P1 | — |
| VFX_CONFETTI | 大胜关卡结算 / 夺冠 / 主界面重大成就 | 彩带纸屑飘落；低端机降级策略：可关 | P1 | — |
| VFX_TRANSITION | 切片之间视角切换（黑幕入 / 出）/ 关卡入场镜头切换 | 黑幕 / 闪白 / 速度线，复用 3.3 黑幕转场定义；**不单独制作新资产** | P1 | — |
| VFX_RED_DOT | 活动内任意可领取入口出现红点时（竞猜页签、BP、商店、每日免费竞猜币等全活动通用） | 入口红点呼吸微光；适用范围对齐 3.7 红点规则章节所列所有红点场景 | P0 | — |

#### 技术约束

- **遮挡禁区**：操作区与轨迹层之上不堆粒子；VFX_GOAL_BURST / VFX_SLICE_FAIL 等成败特效不得遮挡「继续 / 回放」按钮。
- **低端机降级**：帧率连续 3 秒低于 30fps 时触发降级——关键反馈（VFX_GOAL_BURST / VFX_SLICE_FAIL / VFX_REWIND / VFX_GK_CATCH）保留，氛围粒子（VFX_CONFETTI / VFX_CAMERA_FLASH / VFX_CHAMPION）可关闭或减粒子数至 50%。
- **回溯清场**：所有局内 VFX 在 VFX_REWIND 触发首帧立即中断并清场，不得与 VFX_REWIND 叠加播放。
- **震屏强度标注**：VFX_KICK_BURST（出脚）/ VFX_POST_SPARK（击框）/ VFX_GOAL_BURST（进球）的震屏幅度须标注具体像素偏移值，避免晕动；对应震动事件见 \[3.2\] `haptic_config`。
- **实现方式**：打击感粒子优先使用序列帧 / Shader Graph；UI 光效（VFX_GK_TIMER / VFX_RED_DOT）可用 Lottie。

## 6\. 修订记录

### 2026-06-11

1. **5.8 特效（VFX）章节新增**：补充 22 条 VFX 条目，按局内球与射门 / 守门 / 切片成败反馈 / 养成商业化竞猜 / 淘汰赛排名氛围转场分组。每条明确触发时机（精确到判定帧或操作阶段）、表现要点与关联音效。
2. **触发条件细化**：VFX_AIM_LINE 补充划线/弹弓两种模式的实时更新逻辑；VFX_BALL_PULL 明确仅弹弓模式触发；VFX_KICK_BURST / VFX_BALL_TRAIL 补充守门切片中对方 AI 出脚场景。
3. **击中门框判定独立**：VFX_POST_SPARK 明确击中门框为独立判定事件（非进球非扑救），关联音效改为 `SFX_HIT_POST`，与 `haptic_config` 中 `hit_post` 事件对应。
4. **切片成功区分**：VFX_GOAL_BURST 仅在进球（score objective）时触发；助攻传球成功（pass_to objective）不触发光爆，仅保留 VFX_BALL_TRAIL 弱化版。
5. **VFX_REWIND 回溯清场规则**：补充残影须为时间倒流方向；VFX_REWIND 触发首帧所有其他局内 VFX 立即中断清场。
6. **技术约束量化**：降级阈值明确为"连续 3 秒低于 30fps"，并列出可关（氛围粒子）/ 不可关（关键反馈）的具体 VFX ID；震屏条目要求标注像素偏移值。
