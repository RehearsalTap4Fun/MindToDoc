...
### Step 0 · 启动前检查更新（MindToDoc vendor 版本已跳过）

本 Skill 已作为 `vendor/skills/guizang-ppt-skill/` vendor 到 MindToDoc 项目仓库内,不是独立 git 仓库,不执行原始的 `git fetch` 自动检查上游更新流程。

上游仓库：https://github.com/op7418/guizang-ppt-skill 。如需同步上游更新,人工比对 diff 后决定是否覆盖本 vendor 副本(参考 `SKILL.md` 维护说明中"专用派生 Skill"一节的同步方式),不在本 Skill 运行时自动检查。
