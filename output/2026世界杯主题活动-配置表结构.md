...
> **列级 SSOT**：以 `output/test-config/ActivitySoccer.xlsx` 页签表头为准（当前 **24** 个策划页签）；本文由 `generate_config_tables_doc.py` 自动生成字段表。  
> **生成时间**：脚本随测试配置同步更新。
...
| 文件 | 说明 | 策划维护 |
|------|------|----------|
| `ActivitySoccer.xlsx` | 活动玩法主配置（24 个策划页签） | 数值 / 关卡 / 活动 |
...
| QualityShow | int | cs | 品质等级展示(配合UI色块) | — |
...
| ReadyTime | string | s | 每日开赛前准备时长(支持h/m后缀) | — |
| BetTime | string | s | 每日竞猜窗口时长 | — |
| MatchTime | string | s | 每日比赛时长 | — |
...
|------|------|--------|------|------------|
| ID | int | cs | 编号 | — |
| winRate_int | int | cs | 胜率万分值(主队;0..10000) | — |
| powerRate_int | int | cs | 战力比万分值(主队/客队;3000..20000) | — |
...
| 10135609 | `ActvSoccer_move_speed_idle` | 0 | [] | 待机移动速度(固定值,单位m/s) |
...
| 10135623 | `ActvSoccer_ball_control_distance` | 0.5 | [] | 停球/控球距离(m) |
