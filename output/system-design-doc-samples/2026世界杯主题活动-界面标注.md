...
> **钉钉落地**：本地 md 定稿 → 按 `system-design-doc` 的 `dingtalk-sync-reference` 同步；图文块按 `ui-annotation-reference` 插入。
...
flowchart TD
    ...
    B -->|否| C[主案 E 节只列子界面清单占位<br/>不写控件级内容]
    B -->|是| D[2. 原图存入 ui-annotation/assets/]
    D --> E[3. 识图+归类+编号<br/>优先对话内多模态看图]
    E --> F[4. PIL 生成 *_annotated.png + .json]
    F --> G[5. 用户审核标注图]
    G -->|不通过| E
    G -->|通过| H[6. 写入本文：左图右文 + key 表]
    H --> I[7. 上传标注图到钉钉]
    I --> J[8. insert 左图右文双列表格 + key 表]
    J --> K[9. 主案 md2jsonml 同步或块级 insert]
```
...
| 步 | 动作 | 产出 | 参考 |
|----|------|------|------|
| 0 | 先完成主案功能规则；界面与功能结论冲突时以功能规则为准 | — | 主案 C 章 |
| 1 | **索图**：无图则 E 节仅占位子界面名，禁止臆造控件 | 子界面清单 | system-design-doc E 节 |
| 2 | 用户提供的 png/jpg 命名为 `{界面ID}.png` 放入 `ui-annotation/assets/` | 原图 | 本文 §命名 |
| 3 | 识图输出 JSON：`screen_desc` + `elements[{id,name,text,cx,cy}]`；**先归类再编号**（同类合并、父子组合并） | `{id}.json` 草稿 | ui-annotation-reference §一、归类 |
| 4 | Pillow 画红圈编号，保存 `{id}_annotated.png` | 标注图 | ui-annotation-reference §二 |
| 5 | **用户对照原图审核**编号/归类/坐标 | 审核通过 | 必做 |
| 6 | 更新本文对应 `# 界面` 节：概述 + 左图右文右栏 + 新建 key 表 | 本地 md | 本文 §单屏模板 |
| 7 | 钉钉 MCP `get_doc_attachment_upload_info` → PUT 上传 `_annotated.png` | `resourceUrl` | ui-annotation-reference §三 |
| 8 | `insert_document_block` 插入左图右文 table + 其下 KEY/中文 表 | 钉钉在线文档 | ui-annotation-reference §四 |
| 9 | 大文档可走 `md2jsonml.py` 占位 `__IMG_n__` 批量补图，或块级 insert | 与主案一致 | dingtalk-sync-reference §五 |

### 硬性约束

- **无图不写控件**；不得凭相似系统补图里没有的元素。
- **功能规则 vs 界面规则**：触发条件、结算、数值进主案；按钮态、红点、文案、布局进本文。
- **新建 key**：右栏说明用 `{{red:ActvSoccer_xxx｜中文}}`；录入表单独两列表；查重见 K1 `k1-localization-reference` / `ActivitySoccerLanguage.xlsx`。
- **坐标与 OCR 为估计值**，完成后必须让用户核对原图。
- 钉钉 `img.src` 必须用上传返回的**完整 `resourceUrl`**，不能用 `resourceId` 拼路径。

### 文件命名

| 类型 | 规则 | 示例 |
|------|------|------|
| 界面 ID | `{模块}-{简述}` 小写连字符 | `create-character-select` |
| 原图 | `ui-annotation/assets/{界面ID}.png` | `create-character-select.png` |
| 标注图 | `{界面ID}_annotated.png` | `create-character-select_annotated.png` |
| 识图 JSON | `{界面ID}_annotated.json` | 与标注图同批 |
| 钉钉占位 | `<!-- IMG: 中文界面名 \| {界面ID} -->` | 供 md2jsonml images sidecar |
...
|------|---------|--------|------|------|----------|------------|
| 创角 | `create-character-select` | 选角 | 待收图 | ☐ | ☐ | ☐ |
| 创角 | `create-nationality-select` | 选国籍 | 待收图 | ☐ | ☐ | ☐ |
| 创角 | `tutorial-slice` | 试训切片 | 待收图 | ☐ | ☐ | ☐ |
| 创角 | `first-contract` | 首签/合同详情 | 待收图 | ☐ | ☐ | ☐ |
| 创角 | `signing-main` | 签约亮相/活动主界面 | 待收图 | ☐ | ☐ | ☐ |
| 关卡 | `level-prebattle` | 战前/联赛关卡 | 待收图 | ☐ | ☐ | ☐ |
| 关卡 | `level-ingame` | 入场/开球/局内切片 | 待收图 | ☐ | ☐ | ☐ |
| 关卡 | `level-rewind-hint` | 回溯/视角教学 | 待收图 | ☐ | ☐ | ☐ |
| 关卡 | `level-settlement` | 切片完成/关卡/排名结算 | 待收图 | ☐ | ☐ | ☐ |
| 养成 | `growth-fame-life` | 知名度/生活等级 | 待收图 | ☐ | ☐ | ☐ |
| 养成 | `growth-contract` | 合同/成就 | 待收图 | ☐ | ☐ | ☐ |
| 积分赛 | `league-schedule` | 赛程/小关卡 | 待收图 | ☐ | ☐ | ☐ |
| 积分赛 | `league-rank` | 积分榜/射手/助攻 | 待收图 | ☐ | ☐ | ☐ |
| 淘汰赛 | `ko-team` | 组队/创建/加入 | 待收图 | ☐ | ☐ | ☐ |
| 淘汰赛 | `ko-manage` | 队伍管理/审批 | 待收图 | ☐ | ☐ | ☐ |
| 淘汰赛 | `ko-bracket` | 海选/淘汰树/开赛弹窗 | 待收图 | ☐ | ☐ | ☐ |
| 竞猜 | `bet-main` | 竞猜主页/下注 | 待收图 | ☐ | ☐ | ☐ |
| 竞猜 | `bet-result` | 竞猜结果/奖励 | 待收图 | ☐ | ☐ | ☐ |

> 下列 **§遗留导入区** 为 MindToDoc 旧稿（元素平铺表，无红圈编号）。收到正式截图后，按 **§单屏模板** 重做并替换对应节。
...
{screen_desc：这一屏做什么、有哪些区块、玩家可操作什么}

#### 左图右文（右栏编号说明）

左列放标注图（钉钉用上传后的 `resourceUrl`）；右列用有序列表 + 二级 bullet：

1. **{要素名}**
   - 界面规则：{显隐/默认态/点击反馈}
   - 文案：{复用 key / {{red:新建 key｜中文}} }
   - 关联功能规则：见主案「{章节}」{规则要点}
...
| KEY | 中文 |
|-----|------|
| {{red:ActvSoccer_xxx_yyy_01}} | 示例文案 |

---

## 遗留导入区（待升级为标准左图右文）

> MindToDoc 旧稿：平铺「图中元素 | 规则说明」表，**无红圈编号**。收到正式截图后按 §单屏模板 重做。
...
| 国旗网格：中、德、巴、葡、阿、比、西、波、瑞、法等 | 国籍为单选；选中后高亮；国籍只影响首签合同池（`contract_pool`）。 |
...
| 文案“球队给你发来了邀请，挑选一支作为职业生涯起点” | 点击确认后进入合同选择（UI 以球队卡片呈现）。 |
| 队伍选择页标题“请选择队伍” | 展示 3 份候选合同；每张卡片的数据来自 `contract` → `team_config` 反查展示字段。 |
| 球队卡：队服图标、国家/联赛标识、球队名、星级、右箭头 | 点击卡片进入合同详情；星级取自 `contract.team_star`（首签固定 1 星）。 |
| 合同详情“我的合同”弹窗 | 显示国家/联赛、球队名（展示）、星级/待遇/赛季目标/奖励（均取自合同配置）、签名输入框。 |
| 合同详情中的 X/确认按钮 | X 返回选择列表；确认签约写入 `current_contract_id`。签名输入如果只是表现，可默认玩家名。 |
- 签约（可以没有）与主界面
...
| name | string | 国籍名称 | 非空 |
| region | enum | 所属地区（决定首签合同选项的展示球队地区） | 枚举，TODO 列表 |
| contract\_pool | int 数组 | 该国籍首签可选合同ID列表 | 引用 contract；第 1 项为全局默认合同，后 2 项为按地区动态生成的 1 星合同 |
...
| current\_contract\_id | int | 当前生效合同 | 引用 contract；首签及每轮联赛换约确认后更新 |
| pending\_contract\_choices | int 数组 | 待选合同列表（联赛换约） | 服务端生成；长度 ≤ `contract_offer_count`；玩家确认后清空 |
...
| group | int | 所属联赛轮次 | = `season_config.season_id`；同 group 的关卡组成该轮小关卡列表，按 `level_id` 升序为通关顺序 |
...
**知名度等级表** `**fame_growth_level**`（配置页签 `ActvSoccerFameGrowthLevelCfg`）

| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| id | int | 编号 | ≥1 |
| level | int | 知名度等级 | ≥1 |
| exp\_required | int | 升级所需知名度经验 | ≥0 |
| contract\_star\_license\_reward | int | 到达该等级获得的**合同星级许可 ID** | 0=无；2–5 引用 `contract_star_license` |
| title\_lc\_key | string | 称号本地化 | 引用语言表 |
| level\_up\_reward | json | 升级一次性奖励 | TODO 数值 |
| level\_up\_effect | json | 升级效果（评分等） | TODO 数值 |

**生活等级表** `**life_growth_level**`（配置页签 `ActvSoccerLifeGrowthLevelCfg`）

| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| id | int | 编号 | ≥1 |
| level | int | 生活等级 | ≥1 |
| exp\_required | int | 升级消耗金币 | ≥0 |
| reward\_fame | int | 升级奖励知名度 | ≥0 |
| contract\_star\_license\_reward | int | 到达该等级获得的**合同星级许可 ID** | 0=无；2–5 引用 `contract_star_license` |
| ticket\_cap | int | 门票上限 | ≥0 |
| ticket\_recover\_min | int | 门票恢复间隔（分钟） | ≥0 |
| free\_rewind | int | 免费回溯次数 | ≥0 |
| extra\_round | int | 额外联赛轮次 | ≥0 |
| level\_up\_effect | json | 升级效果（评分/buff 等） | TODO 数值 |

**许可奖励配置示例**（双线 OR 取 `max` 生效）

| 表 | 等级 | contract\_star\_license\_reward | 说明 |
|----|------|--------------------------------|------|
| fame\_growth\_level | 5 | 3 | 知名度 5 级获得 3 星许可 |
| life\_growth\_level | 4 | 3 | 生活 4 级获得 3 星许可 |
| — | — | — | 玩家知名度 3 + 生活 4 → `max(2,3)=3`，按 3 星许可抽样 |
...
| team\_star\_factor | float | 合同星级系数（读取 `current_contract.team_star`） | TODO 数值 |

**合同表** `**contract**`（**玩法生效主表**；配置页签 `ActvSoccerContractCfg`，**读取端 `cs`**——待遇/赛季目标/星级等需客户端展示）

| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| contract\_id | int | 合同ID（**发放/签约/存档均关联此 ID**） | ≥1 |
| team\_id | int | 关联展示球队 | 引用 team\_config；仅用于 UI 反查队名/队服/队标/地区 |
| team\_star | int | 合同星级（**唯一星级来源**，参与待遇/知名度/评分） | ≥1 |
...
| grant\_fame\_level / grant\_life\_level | int | 进入抽样池所需的知名度/生活等级 | ≥1 |
| grant\_scene | enum | 发放场景 | `first_sign` 仅首签池 / `league_finish` 参与联赛换约抽样 |

**合同星级许可表** `**contract_star_license**`（配置页签 `ActvSoccerContractStarLicCfg`；联赛换约权重，**2–5 星共 4 档**）

| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| license\_id | int | 许可ID（= 本档最高可出星级） | 2–5 |
| allowed\_team\_stars | int 数组 | 本许可允许出现的合同星级列表 | 升序；如 3 星许可为 \[1,2,3\] |
| star\_weights | int 数组 | 与 `allowed_team_stars` **等长**、按下标对应的抽样权重 | ≥0；如 \[10,30,60\] |

示例（3 星许可，`license_id=3`）：允许 1/2/3 星合同进入星级抽样，权重 10/30/60，即 3 星占比最高但仍可能出低星。

**联赛换约抽样规则**（服务端）

1. `effective_license_id = max(fame_growth_level[fame_lv].contract_star_license_reward, life_growth_level[life_lv].contract_star_license_reward)`（**OR 取较高许可**）
2. 若 `effective_license_id < 2`：仅在 `team_star=1` 的 `league_finish` 合格合同中均匀抽取 `contract_offer_count` 份。
3. 若 `effective_license_id ≥ 2`：取 `license = contract_star_license[effective_license_id]`；对每个候选位：
   - 在 `license.allowed_team_stars` 中筛出 `star ≤ effective_license_id` 且合格池非空的星级，按对应 `star_weights` 加权随机得 `target_star`；
   - 在合格池中 `team_star=target_star` 的合同里均匀随机 1 条，加入 `pending_contract_choices`（不放回）。
4. 客户端展示 `pending_contract_choices`（UI 仍为球队卡）；玩家确认后写 `current_contract_id` 并清空 pending。

**球队配置表** `**team_config**`（**纯展示表**）

| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| team\_id | int | 球队ID | ≥1 |
| name | string | 球队名 | 非空 |
| kit\_key / badge\_key | string | 队服/队标资源键 | 非空 |
| region | enum | 所属地区（展示用） | 关联 nationality\_config.region |

> `team_config` **不含星级字段**。PVE 对手展示与 AI 难度分别见 `level_config.opponent_team_id`（展示）与 `opponent_team_star`（数值），与玩家合同体系无关。
...
**大关卡（联赛轮次）表** `**season_config**`（配置页签 `ActvSoccerSeasonCfg`）

| 字段 | 类型 | 说明 | 取值范围 |
|------|------|------|------------|
| season\_id | int | 大关卡/赛季ID（= `level_config.group`） | ≥1 |
| league\_name\_lc\_key | string | 联赛名称本地化 | 引用语言表 |
| group | int | 联赛系列标识 | ≥1；**总轮次 = count(同 group 的 season 条数)** |
| next\_season | int | **后置联赛** | 完成后进入的 `season_id`；0=本系列结束 |
| contract\_offer\_count | int | 换约候选合同份数 | 默认 3；0=读全局常量 |

> 小关卡列表**不再枚举**：程序查询 `level_config` 中 `group == season_id` 的全部关卡，按 `level_id` 升序组成该轮赛程。完成任意联赛轮次**必然换约**，无需 `contract_on_finish` 字段。

**关联示例**

| season\_id | group | next\_season | 小关卡（level.group） |
|------------|-------|--------------|----------------------|
| 1 | 1 | 2 | 1, 2 |
| 2 | 1 | 3 | 3 |
| 3 | 1 | 0 | 10 |

同 `group=1` 共 3 条 → UI 显示「第 X/3 轮」；正式配置可扩至 32 条。
...
### 4.1 系统交付清单
- [ ] **3.1 创角与新手引导**：4角色仅外观/国籍影响首签合同池/试训3切片(模式可逐切片切换·锁定唯一例外)/选择合同(UI展示为球队卡)·写入current_contract_id
...
- [ ] **3.4 球星养成**： 知名度/生活双线·合同为生效实体(team_config纯展示)·current_contract_id驱动待遇/星级/赛季目标·联赛换约=contract_star_license星级权重+合同均匀随机·货币(金币/门票/竞猜币)
- [ ] **3.5 积分赛**： season/level通过Group关联·总轮次=count(同group)·next_season链推进·小关卡按level_id顺序解锁·完成必然换约
...
- [ ] **两条养成线**：各等级经验曲线、升级奖励与效果、contract\_star\_license\_reward（知名度/生活分别配置、OR取max）
- [ ] **知名度获取公式**：胜负/进球/助攻系数 \+ 当前合同 `team_star` 系数
- [ ] **合同**：各档待遇数值、赛季目标阈值(含type/threshold/settle\_at)、grant_level门槛、contract_offer_count
- [ ] **合同星级许可表**：2–5 星四档、allowed\_team\_stars 与 star\_weights 等长数组及示例权重(如3星许可10/30/60)
...
- [ ] 数据流：关卡结算→养成(知名度,读取current_contract.team_star)→联赛完成→服务端加权生成pending_contract_choices→玩家确认contract_id→评分→淘汰赛→竞猜→排名
...
- [ ] 目标钉钉文档 nodeId 已确认（`get_document_info`）
- [ ] 每张 `_annotated.png` 已上传且 `resourceUrl` 可加载
- [ ] 左图右文表：左 `#E8F2FE`、右 `#FFFAE5`；`colsWidth:[250,400]`
- [ ] 新建 key 紧跟其下 KEY/中文 两列表
- [ ] 主案正文仍不含具体 key 名（key 只在本文与语言表）
- [ ] `list_document_blocks` 抽查：图文为 table 而非 columns
- [ ] 限流/超时后先查块是否已写入再重试
...
| 文档 | 关系 |
|------|------|
| `2026世界杯主题活动.md` | 主功能案；E 节指向本文 |
| `2026世界杯主题活动-配置表结构.md` | 界面文案 key 最终进 `ActvSoccerLanguageCfg` |
| `system-design-doc/references/ui-annotation-reference.md` | 识图、PIL、钉钉上传、JSONML 骨架 |
| `system-design-doc/references/dingtalk-sync-reference.md` | md2jsonml、补图、大文档策略 |
| `system-design-doc/references/k1-localization-reference.md` | key 命名与 cn.tsv 查重 |
