...
> **列级 SSOT**：以 `output/test-config/ActivitySoccer_preview.xlsx` 页签表头为准（当前 **28** 个策划页签）；本文由 `generate_config_tables_doc.py` 自动生成字段表。
> **生成时间**：脚本随测试配置同步更新。
...
| 类型 | 示例 | 说明 |
|------|------|------|
| 玩家存档 | player＿profile、current＿contract＿id、tutorial＿done | 运行时持久化，不进策划表 |
| 局内/关卡运行时 | slice＿runtime、attempt＿id、level＿result | 服务端/客户端内存态 |
| 联赛/排行运行时 | league＿progress、league＿rank＿entry | 服务端计算与存储 |
| 淘汰赛运行时 | knockout＿team、knockout＿match、qualifier＿group＿standing | 组队/对阵/结果由服务生成 |
| 竞猜运行时 | bet＿record、bet＿stats、动态 win＿rate/display＿mult | 下注与派彩过程数据 |
| 程序只读枚举/类型 | slice＿type＿def（L1）、operation＿mode、player＿ai＿duty | 程序定类型与判定；策划在 preset/instance 引用类型名 |
| 客户端表现映射(FSM 状态→动画) | ActvSoccerCharacterStateCfg | 客户端+美术维护;虽在活动 xlsx 中,读取端仅 c,不由数值策划直接编辑数值 |
| 知名度结算 | 独立 fame＿gain＿rule 表 | 由 ContractCfg 的 PayFinish/PayGoal/PayAssist/PayFame 按比赛结果结算 |
| 淘汰赛演算 | ActvSoccerMatchSimulationCfg | 算法口径 match＿simulation＿rule 写在主策划案规则节；可调系数走 KnockoutCfg（如 BotPlayerRating） |
| 竞猜币投放 | ActvSoccerBetCoinSourceCfg | 礼包走通用 GiftCfg.Reward；每日免费见附录 ActvSoccer＿bet＿daily＿free＿amount |
| 竞猜场次运行时 | ActvSoccerBetMatchCfg | 对阵/胜率/展示倍率由服务端按赛程生成 |
| 接口/协议/DB | 字段名、proto、API | 技术派生文档 |

**不进策划 xlsx 的程序/运行时页签**（共 3 个）：`ActvSoccerSliceTypeDefCfg`、`ActvSoccerPlayerAiDutyEnumCfg`、`ActvSoccerBetMatchCfg`。

**走项目通用配套表**（不在 `ActivitySoccer_preview.xlsx` 重复维护）：`ActvSoccerBattlePassCfg` → `BattlePassNew / ActivityBattlePass`、`ActvSoccerExchangeShopCfg` → `ExchangeShopItemCfg`、`ActvSoccerGiftCfg` → `GiftCfg / D2GiftCfg`、`ActvSoccerRankSectionCfg` → `ActivityRank / ActvRankSectionCfg`。
...
| 文件 | 说明 | 策划维护 |
|------|------|----------|
| ActivitySoccer＿preview.xlsx | 活动玩法主配置（28 个策划页签） | 数值 / 关卡 / 活动 |
...
| 页签 | 用途 | 主要关联 | 填表 |
|------|------|----------|------|
| ActvSoccerCharacterCfg | 4 角色外观、球衣与号码 | → 创角流程 | 数值+美术 |
| ActvSoccerNationalityCfg | 国籍与首签候选合同池 | ContractPool→ContractCfg；NameLcKey→Language | 数值 |
| ActvSoccerAchieveCfg | 活动成就(类型/计数/品质/图标/标题/描述) | Type/Count 定义解锁条件；Title/Des 直接展示 | 数值+美术 |
| ActvSoccerSlicePresetCfg | 切片摆位预设（L2 主资产） | SliceType；被 SliceInstanceCfg.PresetID 引用 | 关卡 |
| ActvSoccerSliceInstanceCfg | 切片实例装配（L3;含胜利目标/AI/机制/夹角/视角） | ←Preset；→LevelCfg.SliceList | 关卡 |
| ActvSoccerHapticCfg | 震动事件强度/图案 | 全局事件映射 | 体验/数值 |
| ActvSoccerLevelCfg | 关卡:切片序列、胜平阈值、门票、对手;FortID 供UI跨轮连续编号 | SliceList→Instance；AiProfileID→Profile；SeasonID→SeasonCfg | 关卡+数值 |
...
| ActvSoccerFameGrowthLevelCfg | 知名度等级、许可、主角评分、金币加成 | PlayerRating→淘汰赛主角评分；档位可拆细；ContractStarLicReward→ContractStarLicCfg | 数值 |
| ActvSoccerLifeGrowthLevelCfg | 生活等级、门票 buff、许可、图标名称 | TicketCap/Recover 与附录常量叠加；不参与主角评分 | 数值+美术 |
| ActvSoccerTeamCfg | 球队展示（队名/队服/队标/国旗） | ←ContractCfg.TeamID；←LevelCfg.OpponentTeamID | 数值+美术 |
| ActvSoccerContractStarLicCfg | 联赛换约星级权重 | ←Growth 两线 ContractStarLicReward | 数值 |
| ActvSoccerContractCfg | 合同待遇、赛季目标、发放场景、球衣/目标描述展示 | TeamID→Team；首签←Nationality；换约←StarLic | 数值+美术 |
...
| ActvSoccerBetMultiplierCfg | 胜率×战力比→左右赔率二维对照表(=程序ChampionOddsCfg) | 同步匹配后按 (WinRate, PowerRate) 查行→OddsLeft/OddsRight 万分值；配合附录 bet＿＊ 常量 | 数值 |
| ActvSoccerBetStakeTierCfg | 投注档位(免费+5档) | 下注弹窗选项；免费档HitPayout=5 | 数值 |
| ActvSoccerMatchRewardCfg | 淘汰赛晋级/出局奖励与邮件 | Turn=KnockoutPhaseCfg.ID；MailID→全局邮件 | 数值+活动 |
...
| ActvSoccerRankCfg | 活动排行榜页签配置(图标/背景/关联段位表) | ContentID→通用 ActivityRank/ActvRankSectionCfg | 活动+美术 |
| ActvSoccerGuideCfg | 试训、签约后强引导与第一关新手引导步骤 | DialogueLcKey→Language；NextID串联步骤 | 关卡/体验 |
| ActvSoccerCharacterStateCfg | 主角/球员表现状态→动画 key 映射(FSM) | 客户端 FSM 状态机与美术动作;不进策划数值填表 | 客户端+美术 |
...
| 引导→切片 | GuideCfg.NextID 串联步骤;GestureDesc/DialogueLcKey 驱动指引 |
...
| 淘汰赛→竞猜 | KnockoutPhaseCfg.BetOpen 排期 → 服务端 bet＿match；演算胜率×战力比 → BetMultiplierCfg + bet＿＊ 常量 |
...
| SliceInstance | 库实例id=tier＊100+type＊10+variant(type1-6; attack 1-5,free＿kick 1-4,corner 1-3,penalty/throw＿in/goalkeep 1-2);101-203=试训/引导;AI字段已并入本表 |
...
- **用途**：试训、签约后强引导与第一关新手引导步骤
- **关联**：DialogueLcKey→Language；NextID串联步骤
- **填表**：关卡/体验

| 字段 | 类型 | 读取端 | 说明 | proto/示例 |
|------|------|--------|------|------------|
| ID | int | c | 引导步骤ID | — |
| GuideID | string | c | 引导组ID:trial/main＿to＿level1/level1＿tutorial | — |
| StepIndex | int | c | 组内步骤序号 | — |
| DialogueLcKey | string | c | 引导文案→ActvSoccerLanguageCfg | — |
| GestureDesc | string | c | 手势动画中文描述;无手势留空 | — |
| FailAction | string | c | 失败处理:none/retry＿step/show＿rewind/continue＿as＿failed | — |
| NextID | int | c | 下一步骤ID;0=结束 | — |
...
| RecommendedModes | string array | c | 建议模式 | ["draw＿line","slingshot"] |
| OpenSecondPoint | int | c | 开启二次选点 | — |

### ActvSoccerSliceInstanceCfg

- **用途**：切片实例装配（L3;含胜利目标/AI/机制/夹角/视角）
- **关联**：←Preset；→LevelCfg.SliceList
- **填表**：关卡
...
| AimYawRange | float | c | 可操作夹角 | — |
| CamYawRange | float | c | 可旋转视角 | — |
...
| AiProfileID | int | c | 难度档→ActvSoccerAiProfileCfg | — |
| GoalkeeperAiID | int | c | 门将AI | — |
| DefenderAiID | int | c | 防守AI | — |
| ShooterAiID | int | c | 射手AI | — |
| ModifierID | int | c | 机制→ActvSoccerAiModifierCfg | — |
| IsGuideAi | bool | c | 引导关AI | — |
| RewindRandom | bool | c | 回溯后重随机 | — |
| OverrideReactionTimeMs | int | c | 覆盖反应时间(0=默认) | — |
...
- **用途**：关卡:切片序列、胜平阈值、门票、对手;FortID 供UI跨轮连续编号
- **关联**：SliceList→Instance；AiProfileID→Profile；SeasonID→SeasonCfg
- **填表**：关卡+数值
...
- **用途**：主角/球员表现状态→动画 key 映射(FSM)
- **关联**：客户端 FSM 状态机与美术动作;不进策划数值填表
- **填表**：客户端+美术

| 字段 | 类型 | 读取端 | 说明 | proto/示例 |
|------|------|--------|------|------------|
| ID | int | c | ID | — |
| StateKey | string | c | 表现状态枚举 | — |
| Duty | int | c | 职责类型 | — |
| AnimKey | string | c | 美术动作key | — |
| Mirror | int | c | 镜像来源ID | — |
| IsLoop | int | c | 是否循环 | — |
| NeedEvent | int | c | 是否需要事件 | — |
| EventKey | string | c | 触球事件名 | — |
| Priority | string | c | 优先级 | — |
...
| GoldRewardAdd | int | cs | 金币收益增加 | — |

### ActvSoccerLifeGrowthLevelCfg

- **用途**：生活等级、门票 buff、许可、图标名称
...
| NationFlag | string | c | 国旗 | NationFlagCfg.ID |
...
| ReadyTime | string | s | 备战时长 | — |
| BetTime | string | s | 下注时长 | — |
| MatchTime | string | s | 比赛时长(3段时间必须保证为24小时) | — |
...
| StartTime | string | cs | 开始UTC | — |
| EndTime | string | cs | 结束UTC | — |
...
- **用途**：淘汰赛晋级/出局奖励与邮件
- **关联**：Turn=KnockoutPhaseCfg.ID；MailID→全局邮件
- **填表**：数值+活动
...
- **用途**：胜率×战力比→左右赔率二维对照表(=程序ChampionOddsCfg)
- **关联**：同步匹配后按 (WinRate, PowerRate) 查行→OddsLeft/OddsRight 万分值；配合附录 bet_* 常量
- **填表**：数值
...
- **用途**：活动成就(类型/计数/品质/图标/标题/描述)
- **关联**：Type/Count 定义解锁条件；Title/Des 直接展示
- **填表**：数值+美术
...
- **用途**：活动排行榜页签配置(图标/背景/关联段位表)
- **关联**：ContentID→通用 ActivityRank/ActvRankSectionCfg
- **填表**：活动+美术
...
| ID | int | cs | 配置ID | — |
| Type | int | cs | 类型 | — |
| ContentID | int | cs | 关联ActiveRankSection | — |
| TabKey | string | c | 页签多语言 | — |
| IconPath | string | c | 图标资源路径 | — |
| SelfItemBgPath | string | c | 自己排名的背景版 | — |
| ItemBgPath | string | c | 排名的背景版 | — |
