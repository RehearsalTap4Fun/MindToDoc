...
- **组队期（固定时间）**：
    - 发起者命名球队名与队服（队标）；
    - 玩家发起组队后，呈现在组队列表中，盟友/好友可申请加入，发起者可通过审批列表统同意或拒绝；
    - 已经加入队伍的成员，发起者可踢出
...
- **海选阶段（固定时间）**：参赛队伍可能很多，先海选筛出 **64 强**。 
    ...
    - **赛程**：海选赛仅持续1天，根据组内队伍数量决定轮次，每小时一轮（胜者进入下一轮），直到决出最终K只队伍
    - **限制**：海选阶段**不开放竞猜**（\[3.7\]）；玩家**只能查看自己队伍的对局与本组排名**，看不到其他组与全局对阵。
    - **未晋级**：海选淘汰的队伍发放安慰奖励（仅道具）。
- **单淘汰阶段（固定时间，单败淘汰）**：
    - **分组**：64 强分8组，每组8支队伍进入单败淘汰树（R=6 轮），系统匹配对手，**所有比赛同时统一结算**，允许轮空。
    - **组内赛制**：8组分别决出第一后作为8强，再进行一轮8前淘汰赛，决出4强和冠亚军；
    - **回放**：随后生成的**模拟切片回放与赛后简报仅供玩家事后观看**（类似录像回放），不影响已定结果。
    - **赛程**：阶段共六天，前三天分8组进行比赛，每天一轮；后三天仅一组进行比赛，每天一轮
    - 本阶段**开放竞猜**（\[3.7\]）。
...
D -->|不满| E[系统球员补位·固定评分]
 ...
 G --> H[组内两两演算·按胜场/进球/总评排名不开放竞猜·仅见己队对局与组内排名]
 H --> I{组内前K名? K=64/G}
 I -->|否| J[海选淘汰·安慰奖道具]
 I -->|是| K[晋级64强]
...
| 图中元素 | 规则说明 |
|------------|------------|
| 顶部阶段轴：组队、海选赛、争霸赛及日期 | 映射为组队期、海选阶段、单淘汰阶段；日期由活动配置下发。 |
...
| max\_rounds | int | 单淘汰轮次 R（64强=6 轮） | 默认 6 |
| team\_phase\_time / qualifier\_phase\_time / knockout\_phase\_time | datetime | 组队/海选/单淘汰各阶段固定时间 | TODO |
...
- [ ] **3.2 切片操作核心**： 划线/弹弓·守门强制划线·首个非守门切片锁定整关/6类切片/3道具/回溯重置·三层配置(type\_def/preset/instance)\+objectives/modifiers正交·叠加优先级固定
...
- [ ] **3.6 淘汰赛**： 中后期开放·组队/海选(均分G组取前K=64强·不开竞猜·仅见己组)/单淘汰(64强R=6轮)·演算闭环(切片→比分/平局破法/轮空/名次)·回放仅展示·不参与排名
