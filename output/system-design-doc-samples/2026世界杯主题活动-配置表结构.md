...
> **列级 SSOT**：以 `output/test-config/ActivitySoccer_preview.xlsx` 页签表头为准（当前 **27** 个策划页签）；本文由 `generate_config_tables_doc.py` 自动生成字段表。
> **生成时间**：脚本随测试配置同步更新。
...
| 客户端表现映射 | character＿state＿config（ActvSoccerCharacterStateCfg） | FSM 状态→动画 key；客户端+美术维护，读取端仅 c |
...
| 文件 | 说明 | 策划维护 |
|------|------|----------|
| ActivitySoccer＿preview.xlsx | 活动玩法主配置（27 个策划页签） | 数值 / 关卡 / 活动 |
