...
✅ TBD-0a:程序方与数值方共建 `scripts/check_preset_consistency.py`,校验所有 preset BallPos.z / pos.z 在 [-60, 0] 范围内。
...
| 整场 x 范围 | x ∈ [-18, 18] |
| 整场 z 范围 | z ∈ [-60, 0] |
| 球门线(away / home) | z = 0 / -60 |
| 中线 | z = -30 |
| 边线(场地侧) | x = ±18 |

✅ TBD-6 / TBD-7:比赛主要发生在敌方半场 **z ∈ [-30, 0]、x ∈ [-18, 18]**。
...
✅ TBD-8:**点球点 = (0, 0, -6.5)**。
✅ TBD-8a:开球时弧外其他球员距球 ≥ **5m**(`circle((0,0,-6.5), 5)` 之外)。
...
✅ TBD-10:中圈半径 = **4.5m**。中心在 (0, 0, -30)(中线)。
...
| 2 free_kick | 防守方人墙 ≥ 5m 距球;BallOwner ∈ home;**人墙间距 ≥ 1.4m**(2×球员半径 + 球直径,确保球穿过) |
| 3 penalty | 球必须在点球点 (0,0,-6.5);门将必须在球门线 z=0 ± 死角厚度;其他球员在大禁区外 + 弧外 5m |
| 4 corner | 球必须在 (±17, 0, -1);防守方距球 ≥ 5m |
...
本协议 v1 确认后,主生成器与现存 50 个 preset 数据需要同步重构。当前 P0/P1 已跟进,生成产物需持续通过 `check_protocol_drift.py` / `check_preset_consistency.py` / `check_xlsx_drift.py`。

| # | 改动 | 位置 | 优先级 | 说明 |
|---|---|---|---|---|
| 1 | `GOAL_CENTER_Z = 58 → 0` | `generate_activity_soccer_test_config.py:775` | P0 | 原点改对方球门 |
| 2 | 18 个 preset 的 z 坐标系反转 | `_build_presets:1223+` | P0 | 现 17 个 preset z∈[30,58](错),preset 1 z∈[-30,-5](对);全部按本协议改为 z∈[-30,0] 量级 |
...
| 6 | 点球点 (0,0,50) → (0,0,-6.5) | preset 3 / 9 | P0 | 协议 §3.4 |
