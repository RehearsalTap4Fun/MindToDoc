---
name: mindtodoc
description: 把移动游戏想法转化为小型单文档需求或完整功能策划案；完整功能案生成 BI 日志、美术、音效需求及配置结构，按条件生成界面标注、Checklist 和测试用例，可选同步钉钉。Use when 用户要把游戏创意整理成开发文档、生成游戏开发文档、game design doc、功能策划案。
---

# MindToDoc

把 `input/`（或既有 `*-开发文档.md`）下的粗糙游戏想法，转化为可评审策划案。

## 本技能定位

- **职责**：从素材到成稿的工作流 —— `input/` 扫描、逐模块澄清、产出路径、迁移旧稿。
- **格式来源**：主案、配置结构、美术及界面规范随附于本项目；BI、音效、Checklist、测试用例按派生流程调用对应专用 Skill。
- **默认产出**：先按 `references/derived-documents-workflow.md` 判定交付模式。小型需求可只交付一份主案；完整功能案交付一份主案与 BI 日志需求、美术需求、音效需求、配置结构四类固定派生，界面标注、Checklist 与测试用例按条件生成。交付前完成联合检查，可选同步钉钉。

---

## SSOT 地图

只在一个地方维护细则，避免同一规则多处漂移。SKILL.md 只负责跨 reference 的工作流与判断分流，规则细节一律下沉到对应 reference：

- `SKILL.md`：工作流、写作判断、跨 reference 的硬性总则、何时读取哪个 reference。
- `references/feature-spec-boundaries.md`：主功能案内容边界——写什么、不写什么、范围信息落点、数值奖励粒度。
- `references/feature-spec-writing.md`：起草顺序、功能/界面规则分工、常见错误。
- `references/spec-minimalism-ladder.md`：生成、检测、精简的模式分流；起草范围判断、成稿前自动精简门禁、保护项与审查输出契约。
- `references/derived-documents-workflow.md`：派生文档类型、固定与条件交付、专用 Skill 入口、生成顺序和变更联动；生成或调整派生时必读。
- `references/art-requirements-reference.md`：独立视觉资产提取与美术需求表规范。
- `references/mobile-system-rules-reference.md`：逐系统交付物判定、红点规则、线上数据兼容、配置表边界。
- `references/k1-common-configs.md`：K1 项目公共配置表清单（活动框架 / 通行证 / 排行 / 礼包 / 邮件 / 联盟 / KVK 等），写派生 `-配置表结构.md` 的「公共 / 外部依赖」时直接挑用。
- `references/ui-annotation-reference.md`：UI 识图、归类、标注、上传、左图右文、界面下方 key 录入表。
- `references/dingtalk-sync-reference.md`：钉钉 MCP、`md2jsonml.py`、payload 限制、jsonml/markdown 双路径、补 ind、补表、补图、无 spreadsheet MCP 时导入本地表格生成钉钉表格（axls）。
- `scripts/md2jsonml.py`：转换行为的可执行真相源。

> **关联仓库**：如有必要可查对应开发仓库印证现有业务逻辑：`C:\Project\K1Game\game` 是服务器仓库。 `C:\Project\K1Client\k1_client` 是客户端仓库。
> **维护说明**：主案规范与 `scripts/md2jsonml.py` 自 system-design-doc 技能 vendor 而来；上游更新时手动核对，保留本项目生成条件。
> **专用派生 Skill（已 vendor 到本项目）**：BI 日志需求、音效需求、开发 Checklist、测试用例这四类派生所依赖的专用 Skill 已整体拷贝至 `vendor/skills/<skill-name>/`，随本仓库分发，clone 后即可直接调用，不依赖本机全局 skill 安装。清单见 `references/derived-documents-workflow.md`。`vendor/skills/slg-testcase-generator/` 下的 `scripts/config.json`、`scripts/token.json`、`K1-testcase-config/config.json`（含 Google OAuth 凭据与 Sheet/Drive 资源 ID）未随仓库分发，只保留同目录 `.example`/`.template` 版本；新设备使用前需参照 example 自行创建并填入真实凭据，且已在 `.gitignore` 中排除防止误提交。上游 skill 更新时需人工比对差异后决定是否同步覆盖 vendor 副本。
> **宣讲 / 演示 Skill（已 vendor 到本项目）**：主案定稿后如需向其他职能（策划/程序/测试/运营等）做方案宣讲，可调用 `vendor/skills/guizang-ppt-skill/`（单文件 HTML 横向翻页 PPT，杂志风/瑞士风两套视觉体系，中文排版规则内置）生成宣讲网页；页面内的流程图、对比图等图表元素可参考 `vendor/skills/diagram-design/`（40 种图表类型的编辑器级设计规范）提升视觉质量。这两个 Skill 不是派生文档生成链路的固定环节，按需触发，不影响完整功能案四类固定派生的生成判定。许可证：`guizang-ppt-skill` 为 AGPL-3.0（本项目私有内部使用，未对外分发或作为网络服务提供，不触发开源回馈义务），`diagram-design` 为 MIT（无使用限制）。`guizang-ppt-skill` 原生的"启动前检查上游更新"逻辑（`git fetch`）已在 vendor 副本的 SKILL.md 中标注跳过，因 vendor 后不是独立 git 仓库；如需同步上游更新需人工比对 diff。
> **多语言 key**：本技能**不绑定**任何项目的 localization 文件。新建 key 时按所在项目的多语言规范命名与查重——K1 用**功能名前缀**（如 `ActvSoccer_`），X1/X15 用 `LC_<页签>_`；项目未提供查重源时在 key 表备注「未查重」，不假装已查。

---

## 产出位置

| 产物 | 路径 | 说明 |
|------|------|------|
| **主功能案** | `output/<功能名>.md` | 变更记录 + 简介 + 关键决策 + 详细规则 + 按需项 |
| **配置表结构**（完整功能案派生） | `output/<功能名>-配置表结构.md` | 仅数值策划 Excel；含文档边界与概览 |
| **界面标注**（条件派生） | `output/<功能名>-界面标注.md` | 复杂界面按需采用左图右文 + 红圈 + key SSOT |
| **界面素材** | `output/ui-annotation/assets/{功能标识}/` | `{界面ID}.png`、`*_annotated.png/json`，按功能隔离 |
| **小型需求效果图** | `output/ui-design/{功能标识}/` | 简单界面效果图直接嵌入主案 |
| **其他派生** | 见 `references/derived-documents-workflow.md` | 完整功能案固定的 BI、美术、音效；经确认的 Checklist、测试用例 |
| **旧版（只读素材）** | `output/<游戏名>-开发文档.md` | 迁移源，新稿不覆盖 |

模板：`templates/doc-structure.md`（主案）、`templates/feature-module.md`（模块章）、`templates/derived-config-tables.md`、`templates/derived-ui-annotation.md`。

---

## 文档粒度（硬性）

- **小型需求 = 一份主案**，简单配置、文案、界面规则与效果图直接写入正文，不为凑齐类型生成独立派生。
- **一个完整功能 = 一份主功能案 + 四类固定派生 + 满足条件的派生**，不按子模块拆成多份独立策划案。
- 类型、格式与生成条件统一按 `references/derived-documents-workflow.md` 执行；不生成技术交接文档。

---

## 边界归属（主案 vs 派生）

**「边界」不都在派生文档。** 写错位置会导致主案缺规则或派生越权：

| 边界类型 | 写什么 | 写在哪 |
|----------|--------|--------|
| **边界情况处理** | 异常态、重复请求、活动结束、断线、满/空/禁用态等 | **主案** — `# 边界情况处理` 或各模块 |
| **数据兼容** | 转服、热更、老客户端、幂等（线上） | **主案** — 总则 + 模块特例 |
| **功能 / 系统边界** | 跨系统划分、依赖（合同 vs 球队、复用 X1 竞猜） | **主案** — `# 关键决策` + 模块规则 |
| **数值与奖励边界** | 触发、领取条件；不写最终数值 | **主案** — 「由配置表定义」 |
| **配置表文档边界** | Excel 含什么、不含存档/运行时/协议 | 完整功能案进入 **派生** `-配置表结构.md`；小型需求的简单常量可写主案 |
| **界面规则边界** | 控件、布局、key、红点表现 | 复杂界面进入 **派生** `-界面标注.md`；小型需求的简单界面规则与效果图可写主案 |

**速记**：玩法异常怎么判 → **主案**；完整功能案的表结构 → **配置表派生**；复杂按钮文案与布局 → **界面标注派生**。小型需求按交付模式收拢到主案。

---

## 文档章节模型（采自 system-design-doc）

| 章节 | MindToDoc 落地 |
|-------------------|----------------|
| A 变更记录 / B 简介 / C 详细规则 | 主案必写；阶段 1–2 |
| 关键决策表 | 主案 `# 关键决策`（含文档粒度、配表边界等） |
| D 手游专项 | 流程图、红点、数据兼容、配置表**名称与用途**（字段进派生） |
| E 界面设计 | 小型需求直接写简单界面规则与效果图；完整功能案保留子界面清单，复杂控件进入界面标注派生 |
| F BI / G 待确认 | 按需；真待定才写 |
| 配置表 | 主案表名/用途；Excel 字段 + 文档边界 → 配置表派生 |
| 界面标注 | **不用 HTML 原型**；供图 → 标准流程 |
| 钉钉 | 本地 md SSOT；按需 `references/dingtalk-sync-reference.md` |

起草前必读：`references/feature-spec-boundaries.md`、`references/feature-spec-writing.md`、`references/spec-minimalism-ladder.md`；手游/红点/兼容/配表 → `references/mobile-system-rules-reference.md`。

---

## 流程

先按 `references/spec-minimalism-ladder.md`「模式与触发」区分生成／重整、仅检测、已授权精简和仅同步。生成／重整必须在交付前自动完成 L1 及必要的 L2，无需用户另提精简；仅检测不改稿，已有修改授权不重复确认。

### 阶段 0 · 读取输入

扫描 `input/` 及用户指定的既有 md/xlsx。若全无素材：提示放入 `input/` 或指定旧稿，停止不硬跑。
扫描 `output/` 是否已有 `*-开发文档.md`。若有则应该作为本次修订的基础。


### 阶段 1 · 确认整体框架

1. **澄清意图**：确认系统名、所属项目、一句话定位、目标玩家行为。信息不足先问，不臆造。
2. **问参考玩法**：主动问“有没有参考游戏的某个玩法作为参考？”有则先梳理骨架，无则走标准设计。
3. **批判性分析**：对参考规则或用户给出的规则主动指出不合理处、优化空间和遗漏边界，给建议而非照抄。
4. **关键决策**：把开放条件、核心循环、与其他系统的边界做成**决策表**（维度｜决策｜关键约束/备注）。有分歧时写明为什么选它。
5. **范围与最小交付审查**：按 `references/spec-minimalism-ladder.md` 对模块和计划交付物走一次判断梯；只让确认要实现的规则进入成稿，不为排除项创建章节或派生文档。

产出：**变更记录、简介、关键决策**；线上项目写 **数据兼容总则**。

### 阶段 2 · 逐模块深入

 **章节顺序**
*A. 变更记录*
表格列固定：版本｜变更类型｜变更内容｜经办人｜时间。变更内容写实，不写"完善规则"这类空话。

*B. 简介*
设计目的：从玩法价值讲清这个系统为什么存在。
功能简述：用分层 bullet 勾出系统由哪几块构成、整体怎么转。

成稿简介只包含设计目的与功能简述；关键决策只列实际采用的实现决策；详细规则只写当前有效的对象、条件、行为与结果。目标玩家、开放条件、系统边界、参考玩法和排除项的具体落点统一按 `references/feature-spec-boundaries.md` 执行，不用相邻章节、决策行或派生说明承接已过滤的排除项。

*C. 详细规则*
按模块清单进行循环：

**确认规则**（必选）—— 触发条件、分支、不满足提示；红点/数据兼容必考虑或显式跳过。
- 按真实对象与阶段拆分规则。
- 每条规则尽量具体到**触发条件 + 各分支 + 不满足提示**。
- 异常分支在对应模块写完整；「边界情况处理」只补跨模块或尚未覆盖的场景，已有规则留具体引用。
- 需要时补文案包装：对外命名、世界观包装、提示语语气。
- 需要时补流程图、线上数据兼容处理。
- 只收用户明确未定的真待定项。内部讨论、草稿记录和未决问题不写进主功能案正文。无真待定项时本节不写。

**判定交付物** —— 流程图、红点、兼容。

**追加主案** —— 配置表字段写入派生；跳过项不写空节。

用户确认 → 下一模块。

### 阶段 3 · 派生文档与成稿检查

**交付分流**：读取 `references/derived-documents-workflow.md`。小型需求完成主案并直接嵌入简单界面效果与配置说明，不生成独立派生；完整功能案生成或更新四类固定交付，并按条件处理界面标注、Checklist 和测试用例。已有同用途文档须核验内容与版本后复用；精简不能取消完整功能案的固定交付。
**界面 E 节**：小型需求直接承载简单界面规则和效果图；复杂界面或完整功能案保留有依据的子界面清单，并按阶段 4 判断是否生成标注。
**派生引用**：只有实际生成派生时，主案才增加 `# 派生文档说明`，且只列真实完成、内容匹配的文件。
按需补主案尚未覆盖的 **边界情况处理**。

**成稿门禁（生成／重整必做）**：全文与本次派生齐备后，按派生流程完成联合检查及 `references/spec-minimalism-ladder.md` 的自动精简门禁。阶段 4 属于本次交付时，在其完成后再做最终检查；无图时可先交付其余已完成文档。阶段 1 的范围筛选、逐模块确认和排版验收不能替代本步；后补派生或修改规则后重新核查受影响内容。

### 阶段 4 · 界面标注（复杂界面或完整功能案，用户提供图片后）

```
供图 → assets/{功能标识}/{界面ID}.png → 识图+归类+编号 → *_annotated.png/json
→ 用户审核 → 写入 -界面标注.md（左图右文 + key 表）→（可选）钉钉
```

小型需求的简单界面若在主案中已经能够明确对应图片、文案与交互，不进入本阶段。其余情况按 `references/ui-annotation-reference.md`、`templates/derived-ui-annotation.md` 执行。无图不写控件；新建 key 按所在项目的多语言规范命名与查重。

**截图标注硬性规则**（从旧版平铺表升级时仍适用）：

1. **先看原图再写规则**；`图中内容` 只写截图可见事实，逻辑推导只进说明列。
2. **标号一一对应**；归类合并后右栏用二级 bullet，禁止平铺每个像素级控件。
3. **禁止编造图中元素**；无图则主案/E 节不写控件级内容。
4. 标注完成后须用户对照原图审核，再写入派生 md / 同步钉钉。

### 阶段 5 · 钉钉落地（按需）

生成／重整的主案通过阶段 3 成稿门禁后，再按 `references/dingtalk-sync-reference.md` + `scripts/md2jsonml.py` 同步。仅同步已有文档时按选定源执行，不借同步自动重整正文；已检查且内容未变化时可复用验收记录。

---

## 外部技能与降级

| 能力 | 优先 | 降级 |
|------|------|------|
| 流程图 | mermaid 技能；可用 `mmdc` 校验语法 | 手写 Mermaid + 注明「未校验」 |
| 界面 | 用户截图 + 多模态识图 + PIL 红圈标注 → `-界面标注.md` | 主案 E 节仅占位子界面清单 |
| 钉钉 | 钉钉 MCP + md2jsonml | 仅本地 md |

---

## 主案 / 派生写作要点

**文件编辑与同步**

- 钉钉 ↔ 本地双向同步必须做转义清洗；纯文字表必须使用 `scripts/md2jsonml.py` 生成的 table sidecar，禁止把源表格原文直接送入钉钉。写入后逐表核对标题、列数和行数，并确认没有异常 task/管道段落。详见 `references/dingtalk-sync-reference.md §四、§六、§七`。

**主案**

- 强调：`**加粗**`、`{{red:红字}}`；成段叙述 + 分层 bullet，不堆碎句。
- 主功能案的功能名只放在 md 文件名和钉钉文档名中，正文不额外写同名总标题；正文真正章节从 H1 开始，例如 `# 变更记录`、`# 简介`、`# 详细规则`。
- 标题用干净的 H1/H2/H3/H4 层级，不手写"一、""1.1"。钉钉落地时由 `list.autoLevel=true` 生成自动编号，H1 从 `1.` 开始。
- 规则项默认用两级无序列表：第一级为规则主题，第二级为条件、分支、提示和结果；同一规则含多个分支时必须拆层，细则见 `references/feature-spec-writing.md`。
- 表格用于变更记录、属性分类、配置投放、边界情况、决策记录、BI 字段、多语言 key。
- 源 md 只使用两种行内强调：`**加粗**` 与 `{{red:红字}}`。转换和钉钉落地细节见 `references/dingtalk-sync-reference.md`。
- 凡设计必附“为什么”。影响实现判定的限制写成对应规则的正向约束，不另设“必须避免的方向”或排除项清单。
- 写法要像人写的策划案：成段叙述与分层 bullet 结合，讲清因果与条件，不堆标签碎句。
- 每个业务原子应在一个详细落点写清楚，其他位置只保留摘要、具体引用或独有展示信息。「待确认」是例外，不是兜底。
- 只有用户明确未定，才标待定；待定项必须写明待谁拍板。
- 不用"待确认"掩盖没问够或没写细；能继续写细就继续写，必须决策才提问。
- 待定项一旦确认，结论必须合并回主功能案，不能只停留在临时记录里。
- 最终生成阶段不应保留未处理的待定项；用户明确要求跟踪的后续事项进入变更记录或交付 Checklist，不在主案设置“本期不处理”或后续范围章节。

自检：如果你正要写「待确认」，先问自己这是用户明确没定，还是你没问、没写细。

**配置表派生**

- 含：Excel 页签/字段、概览、公共依赖；不含存档/运行时/协议。
- 「公共 / 外部依赖」表直接从 `references/k1-common-configs.md` 挑表填入；命中即用，不复制内容到派生文档。

**界面标注派生**

- 含：标准流程、子界面进度表、单屏左图右文模板；旧稿平铺表标「遗留，待升级」。

**自由格式派生（美术需求等，无专用 Skill 固定格式时）**

- 只写需求正文本身（资产清单、复用清单等）和待确认项；不写整理日期/来源文档/规则依据等元信息段落，不写"结论先行"式过程叙述，不写"交付格式建议"或"关联文档"链接列表——判断过程是起草动作不是交付内容，关联文档已在主案「派生文档说明」承接。命名建议、规格等零碎信息并入表格备注列，暂无法确定的信息折进待确认项。细则与判断准则见对应 reference（如 `references/art-requirements-reference.md`）。
- 生成 BI 日志需求、音效需求、Checklist、测试用例前先确认 `vendor/skills/` 下是否有对应专用 Skill 并读它的 SKILL.md，不能凭经验自编格式；目标格式为钉钉表格但当前环境没有 spreadsheet 读写 MCP 时，改走 `references/dingtalk-sync-reference.md`「导入本地表格生成钉钉表格」的本地生成 + 导入路径，不退回成 adoc 贴 markdown 表格充数。

---

## 决策变更追溯

长讨论中常会推翻主案已落地的决策（例如本次"整关锁定模式"被改为"默认模式 + 随时可改"）。同一规则在主案的关键决策表、详细规则、流程图、配置表派生中往往出现 3–5 处，临时改一处易漏。

**约定**：对话中一旦确认推翻已写入主案的某条决策，**先写"决策变更日志"再动主案**：

- 位置：长讨论的过程档（如 `output/<功能名>-<专题>-讨论稿.md`）末尾的「主案待修订点」表。
- 格式：列「文件 ｜ 章节 ｜ 旧表述 ｜ 新表述要点」，每个受影响章节一行。
- 用法：作为修订主案的 checklist；改完一行勾一行。
- 主案落地后，把该日志合并进变更记录表的对应版本备注，过程档保留为档。

**判定阈值**：仅在"决策已写入主案"且"修订涉及 ≥ 2 个章节"时使用；单点小改不需要这套流程。

**讨论稿过程档**：超过一节（约 4–6 个共识维度）的长讨论建议 `output/<功能名>-<专题>-讨论稿.md` 边谈边沉淀；每完成一节就 append 共识小结表，避免上下文压缩后丢失。最终成稿合并入主案后，过程档不删除、保留作档。

---

## 自检清单

**整体**

- [ ] 工作流走完：澄清意图、问参考玩法、批判性分析、关键决策、逐章节判定交付物、起草自检、按需落地。
- [ ] 没有用"待确认"掩盖没问够或没写细；真待定项标明待谁拍板；未定数值写"配置"，没编数字。
- [ ] 涉及对应领域时，已读取并按 reference 落地：手游按需项 → mobile reference；界面 → 先索图二选一 + ui-annotation reference；多语言 → 按所在项目多语言规范命名查重；钉钉 → dingtalk reference。
- [ ] 主功能案面向外部读者，没塞内部讨论、技术方案、AI 视图等派生内容。
- [ ] 写法成段、有因果、可评审，无碎句堆砌和口水话。

**主案**

- [ ] ABC + 关键决策齐全；边界情况 / 数据兼容
- [ ] 生成／重整已执行成稿门禁：详细规则无多处完整副本，概览只留摘要或独有信息，已清理讨论残留。
- [ ] 合并前后的主体、触发、条件、动作、分支、反馈、重置和异常对应一致；保护项及派生迁移未闭环项仍有落点。
- [ ] 无“目标与范围 / 目标与对标 / 目标与参考 / 非目标”独立章节；范围信息已按内容性质落入设计目的、关键决策或详细规则
- [ ] 无完整字段 schema、无平铺 UI 大表、规则层无 key 名
- [ ] 待确认仅真待定项

**派生**

- [ ] `-配置表结构.md` 含文档边界与概览；无运行时/存档/协议
- [ ] 已按派生流程判定交付模式；小型需求主案自包含且没有为凑类型生成派生，完整功能案四类固定派生均已生成或核验复用
- [ ] 简单界面效果图已在小型需求主案中对应规则；独立界面标注仅在复杂界面或完整功能案需要且用户供图后生成；Checklist、测试用例已有用户确认；未生成技术交接
- [ ] 按专用 Skill 完成各类校验；主案与派生规则、引用和版本一致
- [ ] 实际存在派生时，`# 派生文档说明` 链接齐全；无派生时不创建空章节

**流程**

- [ ] 阶段 1 问过参考玩法
- [ ] 已对照本节自检清单走完
